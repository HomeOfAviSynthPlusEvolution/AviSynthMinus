...
inline std::string temporal_soften_variant_name(const KernelCpuProfile& variant) {
...
inline std::string temporal_soften_case_name(const char* format, std::size_t width, std::size_t pitch,
                                             int source_planes, std::uint8_t threshold,
                                             const KernelCpuProfile& variant) {
  std::ostringstream stream;
  stream << format << "_Width" << width << "_Pitch" << pitch << "_Planes" << source_planes << "_Threshold"
         << static_cast<unsigned>(threshold) << "_Pattern"
         << (threshold == 255U ? "AverageAnchors" : "ThresholdEdges") << "_"
         << temporal_soften_variant_name(variant);
  return stream.str();
}

inline std::string temporal_soften16_case_name(std::size_t width, std::size_t pitch, int source_planes,
                                               std::uint8_t threshold, int bits_per_pixel,
                                               const KernelCpuProfile& variant) {
  std::ostringstream stream;
  stream << temporal_soften_case_name("Plane16", width, pitch, source_planes, threshold, variant) << "_Bits"
         << bits_per_pixel;
  return stream.str();
}

inline TemporalSoften8Case make_temporal_soften8_case(std::size_t width, std::size_t pitch, int source_planes,
                                                      std::uint8_t threshold, KernelCpuProfile variant,
                                                      std::string expected_hash) {
  TemporalSoften8Case result{
      width, pitch, source_planes, threshold, std::move(variant), std::move(expected_hash), {}};
  result.name = temporal_soften_case_name("Plane8", result.width, result.pitch, result.source_planes,
                                          result.threshold, result.variant);
  return result;
}
...
inline TemporalSoften16Case make_temporal_soften16_case(std::size_t width, std::size_t pitch,
                                                        ...
                                                        std::string expected_hash) {
  TemporalSoften16Case result{
      width, pitch, source_planes, threshold, bits_per_pixel, std::move(variant), std::move(expected_hash),
      {}};
  result.name = temporal_soften16_case_name(result.width, result.pitch, result.source_planes,
                                            result.threshold, result.bits_per_pixel, result.variant);
  return result;
}

inline void PrintTo(const TemporalSoften8Case& test_case, std::ostream* stream) { *stream << test_case.name; }
...
template <typename T>
void fill_temporal_soften_inputs(PlaneView<T> current,
                                 const std::array<std::unique_ptr<GuardedVideoBuffer<T>>, 3>& sources,
                                 int source_planes, std::uint32_t max_value, std::uint32_t threshold,
                                 bool max_threshold) {
...
template <typename T>
void apply_temporal_soften_reference(PlaneView<T> destination,
                                     const std::array<std::unique_ptr<GuardedVideoBuffer<T>>, 3>& sources,
  ...
  for (std::size_t x = 0; x < destination.width(); ++x) {
    ...
    const auto rounded =
        quotient + (remainder * 2U > divisor || (remainder * 2U == divisor && (quotient & 1U) != 0U));
...
void run_temporal_soften_case(const Case& test_case, Function function, std::uint32_t max_value,
  ...
  for (int plane = 0; plane < test_case.source_planes; ++plane) {
    ...
    plane_pointers[source_index] = reinterpret_cast<const BYTE*>(sources[source_index]->view().data());
    source_snapshots[source_index] = sources[source_index]->snapshot_active();
  }

  function(reinterpret_cast<BYTE*>(actual.view().data()), plane_pointers.data(), test_case.source_planes);
...
inline void run_temporal_soften8_case(const TemporalSoften8Case& test_case) {
  run_temporal_soften_case<std::uint8_t>(
      ...
      [&](BYTE* current, const BYTE** sources, int source_planes) {
        ASSERT_EQ(aif_focus_temporal_line(current, sources, source_planes, static_cast<int>(test_case.width),
                                          8, AIF_FOCUS_PLANAR, test_case.threshold, test_case.threshold,
...
inline void run_temporal_soften16_case(const TemporalSoften16Case& test_case) {
  ...
  run_temporal_soften_case<std::uint16_t>(
      test_case,
      [&](BYTE* current, const BYTE** sources, int source_planes) {
        ASSERT_EQ(aif_focus_temporal_line(current, sources, source_planes,
                                          static_cast<int>(test_case.width * sizeof(uint16_t)),
                                          test_case.bits_per_pixel, AIF_FOCUS_PLANAR, test_case.threshold,
                                          test_case.threshold, test_case.variant.cpu),
                  AIF_FOCUS_OK);
      },
      max_value, threshold);
}
