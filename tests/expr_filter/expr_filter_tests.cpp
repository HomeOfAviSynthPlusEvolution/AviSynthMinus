...
void fill_seeded_frame(PVideoFrame &frame, const VideoInfo &video_info,
                       std::uint32_t seed, std::uint32_t frame_salt) {
  for (const int plane : video_frame_planes(video_info)) {
    fill_plane_full_pitch(frame, static_cast<std::uint8_t>(0xa5 + plane),
                          plane);
    ...
    fill_random(view, seed ^ frame_salt ^
                          static_cast<std::uint32_t>(plane) * 0x9e3779b9U);
  }
}

std::vector<PVideoFrame>
make_seeded_yuv_frames(AviSynthEnvironment &environment,
                       const VideoInfo &video_info) {
  std::vector<PVideoFrame> frames;
  for (int frame_index = 0; frame_index < video_info.num_frames;
       ++frame_index) {
...
std::vector<PVideoFrame>
make_seeded_rgb_frames(AviSynthEnvironment &environment,
                       const VideoInfo &video_info) {
  std::vector<PVideoFrame> frames;
  for (int frame_index = 0; frame_index < video_info.num_frames;
       ++frame_index) {
...
std::vector<PVideoFrame>
make_seeded_lut_frames(AviSynthEnvironment &environment,
                       const VideoInfo &video_info) {
  std::vector<PVideoFrame> frames;
  for (int frame_index = 0; frame_index < video_info.num_frames;
       ++frame_index) {
...
PClip make_filter(const PClip &clip,
                  const std::vector<std::string> &expressions,
                  const IrisTestBackend &variant, int lutmode,
                  IScriptEnvironment *environment) {
  return make_iris_test_filter({clip}, expressions, variant, lutmode, nullptr,
                               environment);
}

class ExprYuvVariantTest : public ::testing::TestWithParam<IrisTestBackend> {};
...
TEST_P(ExprYuvVariantTest, EvaluatesSeededYuvPlanesAcrossTailWidth) {
  ...
  PClip filter =
      make_filter(source, expressions, variant, 0, environment.get());
  ...
  for (const int plane : {PLANAR_Y, PLANAR_U, PLANAR_V}) {
    for (int y = 0; y < height; ++y) {
      ...
      for (int x = 0; x < width; ++x) {
        const int coordinate_term = plane == PLANAR_Y   ? x
                                    : plane == PLANAR_U ? y
                                                        : x + y;
        const auto expected = static_cast<std::uint8_t>(
            std::min(255, source_row[x] + coordinate_term));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn(iris_test_backends()),
    [](const ::testing::TestParamInfo<IrisTestBackend> &info) {
      return info.param.name;
    });

class ExprRgbVariantTest : public ::testing::TestWithParam<IrisTestBackend> {};
...
TEST_P(ExprRgbVariantTest, PreservesPlanarGbrOrderAndCopiesAlphaAtSixteenBits) {
  ...
  const FrameSnapshot source_snapshot =
      FrameSnapshot::capture(frames[0], video_info);
  auto *source_impl = new FrameSequenceClip(video_info, frames);
  const PClip source(source_impl);
  ...
  for (int y = 0; y < height; ++y) {
    for (const int plane : {PLANAR_R, PLANAR_G, PLANAR_B}) {
      ...
      const auto *output_row = reinterpret_cast<const std::uint16_t *>(
          output->GetReadPtr(plane) + y * output->GetPitch(plane));
      const std::uint16_t bias = plane == PLANAR_R   ? 101
                                 : plane == PLANAR_G ? 203
                                                     : 307;
      for (int x = 0; x < width; ++x) {
        const auto expected =
            static_cast<std::uint16_t>(std::min<std::uint32_t>(
                65535U, static_cast<std::uint32_t>(source_row[x]) + bias));
        EXPECT_EQ(output_row[x], expected)
            << "variant=" << variant.name << " plane=" << plane << " x=" << x
...
class ExprLut : public ::testing::TestWithParam<IrisTestBackend> {};

TEST_P(ExprLut, BuildsOneDimensionalTableAndUsesRequestedFrame) {
  AviSynthEnvironment environment;
  const auto video_info =
      make_video_info(VideoInfoSpec{19, 2, VideoInfo::CS_Y8, 2, 25, 1});
  auto frames = make_seeded_lut_frames(environment, video_info);
  const FrameSnapshot first_snapshot =
      FrameSnapshot::capture(frames[0], video_info);
  const FrameSnapshot second_snapshot =
      FrameSnapshot::capture(frames[1], video_info);
  auto *source_impl = new FrameSequenceClip(video_info, frames);
  const PClip source(source_impl);
  ...
  PClip filter =
      make_filter(source, expressions, GetParam(), 1, environment.get());
  ...
  for (int y = 0; y < video_info.height; ++y) {
    const auto *source_row =
        frames[1]->GetReadPtr(PLANAR_Y) + y * frames[1]->GetPitch(PLANAR_Y);
    const auto *output_row =
...
INSTANTIATE_TEST_SUITE_P(
    Backends, ExprLut, ::testing::ValuesIn(iris_test_backends()),
    [](const ::testing::TestParamInfo<IrisTestBackend> &info) {
      return info.param.name;
    });
