...
void PrintTo(const CropArgumentCase& test_case, std::ostream* output) {
...
PClip create_point_resize(PClip source, const CropArgumentCase& test_case, IScriptEnvironment* environment) {
  ...
  return FilteredResize::Create_PointResize(AVSValue(args.data(), static_cast<int>(args.size())), nullptr, environment)
      .AsClip();
}
...
TEST_P(PointResizeFactory, RejectsScriptGeneratedNanCropArgument) {
  ...
  ASSERT_TRUE(std::isnan(nan.AsFloat())) << "crop argument=" << test_case.name;
  EXPECT_THROW(create_point_resize(source, test_case, environment.get()), AvisynthError)
      << "crop argument=" << test_case.name;
  EXPECT_EQ(FrameSnapshot::capture(source_frame, vi), source_before)
      << "crop argument=" << test_case.name << " modified its source";
}

INSTANTIATE_TEST_SUITE_P(BoundaryCases, PointResizeFactory,
