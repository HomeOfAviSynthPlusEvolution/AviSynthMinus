...
StaticVideoSource make_yuv444_source(AviSynthEnvironment& environment, std::uint8_t luma, std::uint8_t chroma_u,
...
std::array<AVSValue, 14> make_overlay_args(const PClip& base, const PClip& overlay, const PClip& mask,
                                           const AVSValue& opacity, const char* mode) {
...
PVideoFrame render_overlay(AviSynthEnvironment& environment, const PClip& base, const PClip& overlay, const PClip& mask,
                           const AVSValue& opacity, const char* mode) {
...
std::vector<int> yuv_planes() {
  return {PLANAR_Y, PLANAR_U, PLANAR_V};
}

::testing::AssertionResult active_planes_equal(const PVideoFrame& expected, const PVideoFrame& actual) {
    ...
    for (int y = 0; y < expected->GetHeight(plane); ++y) {
      ...
      for (int x = 0; x < expected->GetRowSize(plane); ++x) {
        if (expected_row[x] != actual_row[x]) {
          return ::testing::AssertionFailure()
                 << "plane=" << plane << " x=" << x << " y=" << y << " expected=" << static_cast<int>(expected_row[x])
...
bool converted_luma_matches(const PVideoFrame& source, const PVideoFrame& destination) {
  for (int y = 0; y < destination->GetHeight(PLANAR_Y); ++y) {
    ...
    const auto* destination_row = destination->GetReadPtr(PLANAR_Y) + y * destination->GetPitch(PLANAR_Y);
...
bool narrow_conversion_matches_reference(const PVideoFrame& source, const PVideoFrame& destination,
  ...
  for (const int plane : {PLANAR_U, PLANAR_V}) {
    for (int y = 0; y < destination->GetHeight(plane); ++y) {
      const auto* destination_row = destination->GetReadPtr(plane) + y * destination->GetPitch(plane);
      for (int x = 0; x < destination->GetRowSize(plane); ++x) {
        ...
        if (operation == NarrowConversion::Yv12ToYv24) {
          ...
          expected = static_cast<std::uint8_t>(
              (static_cast<int>(source_row[2 * x]) + static_cast<int>(source_row[2 * x + 1]) + 1) / 2);
        }
...
bool run_narrow_conversion(const NarrowConversionCase& test_case) {
  ...
  const int source_pixel_type = test_case.operation == NarrowConversion::Yv12ToYv24   ? VideoInfo::CS_YV12
  ...
  const auto source_info = make_video_info(VideoInfoSpec{kWidth, kHeight, source_pixel_type, 1, 25, 1});
  const auto destination_info = make_video_info(VideoInfoSpec{kWidth, kHeight, destination_pixel_type, 1, 25, 1});
  ...
  return narrow_conversion_matches_reference(source, destination, test_case.operation) && source->CheckMemory() != 1 &&
         destination->CheckMemory() != 1;
}
...
TEST_P(NarrowOverlayConversions, HandlesLegallyNarrowRowsWithoutCorruptingFrames) {
  ...
  EXPECT_TRUE(run_narrow_conversion(test_case));
}

INSTANTIATE_TEST_SUITE_P(BoundaryCases, NarrowOverlayConversions,
...
TEST_P(OverlayOpacityConstruction, RejectsNonFiniteOpacityBeforeIntegerScaling) {
  ...
  const AVSValue opacity = test_case.value == NonFiniteOpacity::Nan ? script_generated_nan(environment.get())
  ...
  EXPECT_THROW(
      { Overlay filter(base.clip, AVSValue(args.data(), static_cast<int>(args.size())), environment.get()); },
      AvisynthError)
      << "Overlay opacity=" << test_case.name;
  EXPECT_EQ(FrameSnapshot::capture(base.frame, base.video_info), base.snapshot)
      << "Overlay opacity=" << test_case.name << " modified base during construction";
  EXPECT_EQ(FrameSnapshot::capture(overlay.frame, overlay.video_info), overlay.snapshot)
      << "Overlay opacity=" << test_case.name << " modified overlay during construction";
  EXPECT_TRUE(base.clip_impl->frame_requests().empty());
  EXPECT_TRUE(overlay.clip_impl->frame_requests().empty());
}

INSTANTIATE_TEST_SUITE_P(BoundaryCases, OverlayOpacityConstruction,
                         ::testing::Values(NonFiniteOpacityCase{"Nan", NonFiniteOpacity::Nan},
                                           NonFiniteOpacityCase{"PositiveInfinity",
                                                                NonFiniteOpacity::PositiveInfinity}),
...
void PrintTo(const OverlayModeCase& test_case, std::ostream* output) {
  *output << test_case.name;
}

PVideoFrame render_mask_equivalence_case(AviSynthEnvironment& environment, const char* mode, bool with_mask,
                                         StaticVideoSource& base, StaticVideoSource& overlay,
                                         StaticVideoSource& full_mask) {
...
TEST_P(EqualLumaMaskEquivalence, TreatsFullMaskLikeOmittedMaskAtEqualLuma) {
  ...
  const PVideoFrame unmasked =
      render_mask_equivalence_case(environment, test_case.mode, false, unmasked_base, unmasked_overlay, full_mask);
  const PVideoFrame masked =
      render_mask_equivalence_case(environment, test_case.mode, true, masked_base, masked_overlay, full_mask);

  EXPECT_TRUE(active_planes_equal(unmasked, masked)) << "Overlay mode=" << test_case.name;
  ...
  EXPECT_EQ(FrameSnapshot::capture(unmasked_base.frame, unmasked_base.video_info), unmasked_base.snapshot);
  EXPECT_EQ(FrameSnapshot::capture(unmasked_overlay.frame, unmasked_overlay.video_info), unmasked_overlay.snapshot);
  EXPECT_EQ(FrameSnapshot::capture(masked_base.frame, masked_base.video_info), masked_base.snapshot);
  EXPECT_EQ(FrameSnapshot::capture(masked_overlay.frame, masked_overlay.video_info), masked_overlay.snapshot);
  EXPECT_EQ(FrameSnapshot::capture(full_mask.frame, full_mask.video_info), full_mask.snapshot);
}

INSTANTIATE_TEST_SUITE_P(BoundaryCases, EqualLumaMaskEquivalence,
                         ::testing::Values(OverlayModeCase{"Darken", "Darken"}, OverlayModeCase{"Lighten", "Lighten"}),
                         [](const ::testing::TestParamInfo<OverlayModeCase>& info) { return info.param.name; });
...
void PrintTo(const MaskEndpointCase& test_case, std::ostream* output) {
  *output << test_case.name;
}
...
TEST_P(FullMaskEndpointEquivalence, MatchesOmittedMaskAcrossIntegerModes) {
  ...
  StaticVideoSource unmasked_overlay = make_yuv444_source(environment, test_case.overlay_luma, 213, 41);
  StaticVideoSource masked_base = make_yuv444_source(environment, test_case.base_luma, 79, 181);
  StaticVideoSource masked_overlay = make_yuv444_source(environment, test_case.overlay_luma, 213, 41);
  StaticVideoSource full_mask = make_yuv444_source(environment, 255, 255, 255);
...
INSTANTIATE_TEST_SUITE_P(BoundaryCases, FullMaskEndpointEquivalence,
                         ...
                         [](const ::testing::TestParamInfo<MaskEndpointCase>& info) { return info.param.name; });
