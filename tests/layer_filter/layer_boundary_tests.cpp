...
TEST(ResetMaskConstruction, RejectsScriptGeneratedNanMaskAndOpacity) {
  ...
  EXPECT_THROW({ ResetMask filter(source.clip, nan.AsFloat(), environment.get()); }, AvisynthError);
  EXPECT_EQ(FrameSnapshot::capture(source.frame, source.video_info), source.snapshot)
      << "ResetMask non-finite construction modified its source";
  EXPECT_TRUE(source.clip_impl->frame_requests().empty()) << "ResetMask non-finite construction requested a frame";
}
...
TEST(ResetMaskFloat, AppliesFiniteMaskWithoutOpacityOverride) {
  ...
  ResetMask filter(source.clip, 0.25F, environment.get());
  ...
  for (int y = 0; y < output->GetHeight(PLANAR_A); ++y) {
    const auto* row = reinterpret_cast<const float*>(output->GetReadPtr(PLANAR_A) +
                                                     static_cast<std::size_t>(y) * output->GetPitch(PLANAR_A));
    for (int x = 0; x < width; ++x) {
      EXPECT_FLOAT_EQ(row[x], 0.25F) << "ResetMask float x=" << x << " y=" << y;
    }
  }
...
StaticVideoSource make_yuy2_source(AviSynthEnvironment& environment, const std::array<std::uint8_t, 4>& luma,
...
TEST(MergeRgb, ExtractsLumaChannelsFromPackedYuy2Inputs) {
  ...
  for (int x = 0; x < 4; ++x) {
    EXPECT_EQ(static_cast<int>(row[4 * x]), static_cast<int>(kBlue[static_cast<std::size_t>(x)]))
        << "MergeRGB blue x=" << x;
    EXPECT_EQ(static_cast<int>(row[4 * x + 1]), static_cast<int>(kGreen[static_cast<std::size_t>(x)]))
        << "MergeRGB green x=" << x;
    EXPECT_EQ(static_cast<int>(row[4 * x + 2]), static_cast<int>(kRed[static_cast<std::size_t>(x)]))
        << "MergeRGB red x=" << x;
    EXPECT_EQ(static_cast<int>(row[4 * x + 3]), 0) << "MergeRGB alpha x=" << x;
  }
  EXPECT_NE(output->CheckMemory(), 1);
...
TEST(LayerMul, PreservesIntegerFullScaleEndpoint) {
  ...
  Layer filter(base.clip, overlay.clip, "Mul", -1, 0, 0, 0, true, 1.0F, 0, environment.get());
  const PVideoFrame output = filter.GetFrame(0, environment.get());
  ...
  for (int y = 0; y < output->GetHeight(PLANAR_Y); ++y) {
    ...
    for (int x = 0; x < output->GetRowSize(PLANAR_Y); ++x) {
      EXPECT_EQ(static_cast<int>(row[x]), 255) << "Layer Mul x=" << x << " y=" << y;
    }
  }
  EXPECT_NE(output->CheckMemory(), 1);
...
TEST(LayerMul, AppliesMaskAtPartialOpacity) {
  ...
  Layer filter(base.clip, overlay.clip, "Mul", -1, 0, 0, 0, true, 0.5F, 0, environment.get());
  const PVideoFrame output = filter.GetFrame(0, environment.get());

  for (int y = 0; y < output->GetHeight(PLANAR_Y); ++y) {
    const auto* row = output->GetReadPtr(PLANAR_Y) + y * output->GetPitch(PLANAR_Y);
    for (int x = 0; x < output->GetRowSize(PLANAR_Y); ++x) {
      EXPECT_EQ(static_cast<int>(row[x]), 200) << "Layer Mul partial opacity with zero mask x=" << x << " y=" << y;
    }
  }
  EXPECT_NE(output->CheckMemory(), 1);
  EXPECT_EQ(base.clip_impl->frame_requests(), std::vector<int>{0});
...
TEST_P(LayerThresholdConstruction, RejectsValuesOutsideEightBitDomain) {
  ...
  EXPECT_THROW(
      {
        Layer filter(base.clip, overlay.clip, "Lighten", -1, 0, 0, test_case.threshold, true, 1.0F, 0,
                     environment.get());
      },
      AvisynthError)
      << "Layer threshold=" << test_case.name;
  EXPECT_EQ(FrameSnapshot::capture(base.frame, base.video_info), base.snapshot)
      << "Layer threshold=" << test_case.name << " modified base source";
  EXPECT_EQ(FrameSnapshot::capture(overlay.frame, overlay.video_info), overlay.snapshot)
      << "Layer threshold=" << test_case.name << " modified overlay source";
  EXPECT_TRUE(base.clip_impl->frame_requests().empty())
      << "Layer threshold=" << test_case.name << " requested base during construction";
  EXPECT_TRUE(overlay.clip_impl->frame_requests().empty())
      << "Layer threshold=" << test_case.name << " requested overlay during construction";
}

INSTANTIATE_TEST_SUITE_P(BoundaryCases, LayerThresholdConstruction,
                         ::testing::Values(LayerThresholdCase{"NegativeOne", -1},
                                           LayerThresholdCase{"AboveByteRange", 256}),
                         [](const ::testing::TestParamInfo<LayerThresholdCase>& info) { return info.param.name; });
...
TEST_P(MaskRgb32NarrowRows, HandlesRowsShorterThanOneVector) {
  ...
  AviSynthEnvironment environment;
  const auto video_info = make_video_info(VideoInfoSpec{width, 1, VideoInfo::CS_BGR32, 1, 25, 1});
  ...
  for (int x = 0; x < width; ++x) {
    const int expected_alpha =
        (3736 * mask_row[4 * x] + 19234 * mask_row[4 * x + 1] + 9798 * mask_row[4 * x + 2] + 16384) >> 15;
    output_is_expected = output_is_expected && output_row[4 * x] == source_row[4 * x] &&
                         output_row[4 * x + 1] == source_row[4 * x + 1] &&
                         output_row[4 * x + 2] == source_row[4 * x + 2] && output_row[4 * x + 3] == expected_alpha;
  }
  EXPECT_TRUE(output_is_expected);
}

INSTANTIATE_TEST_SUITE_P(BoundaryCases, MaskRgb32NarrowRows, ::testing::Values(1, 2, 3),
                         [](const ::testing::TestParamInfo<int>& info) {
                           return "Width" + std::to_string(info.param);
                         });

PVideoFrame make_yuv_frame(AviSynthEnvironment& environment, const VideoInfo& video_info, std::uint8_t luma) {
  PVideoFrame frame = environment.get()->NewVideoFrame(video_info);
  fill_plane_full_pitch(frame, luma, PLANAR_Y);
  ...
  for (const int plane : {PLANAR_Y, PLANAR_U, PLANAR_V}) {
    ...
    for (int y = 0; y < output->GetHeight(plane); ++y) {
      ...
      for (int x = 0; x < output->GetRowSize(plane); ++x) {
        EXPECT_EQ(row[x], expected) << "Subtract plane=" << plane << " x=" << x << " y=" << y;
      }
    }
  }
