...
TEST(SimdDispatchTest, NativeTargetExecutesCorrectlyOrFallsBack) {
  ...
  const int native_flags = env.get()->GetCPUFlags();
...
TEST(SimdDispatchTest, HandlesSmallAndNonVectorLengthsWithoutOverrun) {
  avsut::test::AviSynthEnvironment env;
  const auto native_fn = probe::ResolveProbeKernel(env.get()->GetCPUFlags());
...
TEST(SimdDispatchTest, MultiEnvironmentInstancesRemainIsolated) {
  ...
  const int flags_native = env_native.get()->GetCPUFlags();
  const int flags_none = env_none.get()->GetCPUFlags();
...
TEST(SimdDispatchTest, NonX86NoneAndNativeSettings) {
  avsut::test::AviSynthEnvironment env;
  EXPECT_EQ(env.get()->GetCPUFlags(), CPUF_FORCE);
  for (const char* setting : {"", "sse2", "avx2", "sse2,avx2+", "native"}) {
    const AVSValue none("  NoNe  ");
    env.get()->Invoke("SetMaxCPU", AVSValue(&none, 1));
    EXPECT_EQ(env.get()->GetCPUFlags(), 0);
    EXPECT_EQ(probe::ResolveProbeKernel(env.get()->GetCPUFlags()), &probe::ProbeKernel_C);

    const AVSValue native(setting);
    env.get()->Invoke("SetMaxCPU", AVSValue(&native, 1));
    EXPECT_EQ(env.get()->GetCPUFlags(), CPUF_FORCE) << setting;
  }
}
#endif
...
TEST(SimdDispatchTest, ConcurrentExecutionsDoNotInterfere) {
  avsut::test::AviSynthEnvironment env;
  const auto fn = probe::ResolveProbeKernel(env.get()->GetCPUFlags());
...
TEST(SimdDispatchTest, MultipleHighwayConsumerTranslationUnitsCoexist) {
  avsut::test::AviSynthEnvironment env;
  const int flags = env.get()->GetCPUFlags();
...
TEST(SimdDispatchTest, SharedConsumerUsesStaticHighwayRuntime) {
  ...
  const char* native_target =
      avs_simd_shared_consumer_target_name(env.get()->GetCPUFlags());
